...
def load_dataset(m):
    ...
    human = dict(zip(sorted(human_vocab) + ['<unk>', '<pad>', '<eos>', '='], 
                     list(range(len(human_vocab) + 4))))
    inv_machine = dict(enumerate(sorted(machine_vocab) + ['<unk>', '<pad>', '<eos>']))
    machine = {v:k for k,v in inv_machine.items()}
 
    return dataset, human, machine, inv_machine

def preprocess_data(dataset, human_vocab, machine_vocab, Tx = 30, Ty = 11):
...
def run_example(model, input_vocabulary, inv_output_vocabulary, text):
    encoded = string_to_int(text, 30, input_vocabulary)
